...
load = do
    ...
    [$p|`ESet new: (pattern: Expression) to: (value: Expression)|] =: do
    ...
    [$p|`EDefine new: (pattern: Expression) as: (expr: Expression)|] =: do
        Expression pat <- here "pattern" >>= findExpression
    ...
    [$p|`EDispatch new: (name: Particle) to: (targets: List)|] =: do
